...
inline uint32_t swap2(uint16_t x) { return (((x & 0xff00) >> 8) | ((x & 0x00ff) << 8)); }
...
pcap_t* pcap_fopen_offline(FILE* fp, char* errbuf) {
    ...
    if (header.magic != 0xa1b2c3d4) {
        snprintf(errbuf, PCAP_ERRBUF_SIZE, "Cannot decode pcap header 0x%x; swapped=%d", header.magic, swapped);
        return 0;
    }
    if (header.version_major != PCAP_VERSION_MAJOR || header.version_minor != PCAP_VERSION_MINOR) {
        snprintf(errbuf, PCAP_ERRBUF_SIZE, "Cannot read pcap version %d.%d", header.version_major,
...
void pcap_break_loop(pcap_t* p) { p->break_loop = true; }
