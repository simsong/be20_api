...
template <> void scanner_config::get_config(const std::string& name, std::string* val, const std::string& help) {
    ...
    if (it != namevals.end() && val) { *val = it->second; }
}

/* signed/unsigned char need cast to larger type for <<
 * Otherwise it is interpreted as a character.
 */
template <> void scanner_config::get_config(const std::string& n, unsigned char* val_, const std::string& help) {
    unsigned int val = *val_;
    get_config(n, &val, help);
    *val_ = val;
}
template <> void scanner_config::get_config(const std::string& n, signed char* val_, const std::string& help) {
    int val = *val_;
    get_config(n, &val, help);
...
template <> void scanner_config::get_config(const std::string& n, bool* val, const std::string& help) {
    std::stringstream ss;
    ss << ((*val) ? "YES" : "NO");
    std::string v(ss.str());
    get_config(n, &v, help);
